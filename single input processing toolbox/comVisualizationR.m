function out = comVisualizationR(int,out2input,out1input,pst,n)
...
ret1(isnan(ret1)) = 0;
%% reliability metric
Rel = (sum(out1input.avgWeg1,3));
Rel(Rel<1) = 1;
rel2 = mat2gray(log(Rel),[3 10]);
Rel = sum(out1input.avgWeg2,3);
Rel((Rel<1)) = 1;
rel3 = mat2gray(log(Rel),[3 10]);
rel23 = (rel2+rel3)/2;
...
correlation = corrIdx(2,1);
xlabel('\Deltan1'); ylabel('\Deltan3'); axis([0,2.5e-3,0,2.5e-3])
...
correlation = corrIdx(2,1);
xlabel('\Deltan2'); ylabel('\Deltan3'); axis([0,2.5e-3,0,2.5e-3])
text(0.2e-3,2.4e-3,sprintf('a=%1.2g',b(2)),'Color','black')
text(0.2e-3,2.2e-3,sprintf('r=%1.2g',correlation),'Color','black')
axis xy; axis square
...
imagesc(retbinsx,retbinsy,log10(hh2')); hold on;axis xy;% colormap(ax1, parula);%axis square
...
imagesc(retbinsx,retbinsy,log10(hh2')); hold on;axis xy;% colormap(ax1, parula);%axis square
xlabel('(\Deltan_1 +\Deltan_3)/2');ylabel('\Deltan_1-\Deltan_3');
set(gca,'Xtick',[0 0.5 1 1.5 2].*1e-3);set(gca,'Ytick',[-1 -0.5 0 0.5 1].*1e-3);axis([0 2.5 -1.5 1.5]*1e-3)
hc = colorbar; set(hc,'Ytick', [])
% calculation
...
imagesc(retbinsx,retbinsy,log10(hh2')); hold on;axis xy;% colormap(ax1, parula);%axis square
xlabel('(\Deltan_2 +\Deltan_3)/2');ylabel('\Deltan_2-\Deltan_3');
set(gca,'Xtick',[0 0.5 1 1.5 2].*1e-3);set(gca,'Ytick',[-1 -0.5 0 0.5 1].*1e-3);axis([0 2.5 -1.5 1.5]*1e-3)
hc = colorbar; set(hc,'Ytick', [])
% calculation
...
width = 34;
rel2 = circInterp(sum(out1input.avgWeg1,3),Nout,Range);
rel3 = circInterp(sum(out1input.avgWeg2,3),Nout,Range);

rel2s = (retfilter(rel2,width,maskC));
rel3s = (retfilter(rel3,width,maskC));
% rel2(rel2<1) = 1;
% rel3(rel3<1) = 1;
% rel2mask = mat2gray((rel2),[0,1000]);
% rel3mask = mat2gray((rel3),[1,1000]);
% h = fspecial('disk',width/2);
% h = h(1:width,1:width);
ret1s = retfilter(ret1,width,maskC);
ret2s = retfilter(ret2,width, rel2, maskC);
ret3s = retfilter(ret3,width, rel3, maskC);
